...
test("Fetch tab by title and artist", async () => {
    ...
    tabs.forEach(
      (tab) =>
        expect(tab.song_title).toBe(title) &&
        expect(tab.artist).toBe(artist) &&
        expect(tab.tab).toBe("some tab") &&
        expect(tab.url).toBe("this is also a url.net")
    );
});
...
test("Adding tab", async () => {
    ...
    const result = await tabServices.addTab(dummyTab.song_title, dummyTab.artist, dummyTab.tab, dummyTab.url);
    expect(result).toBeTruthy();
    expect(result.song_title).toBe(dummyTab.song_title);
    expect(result.artist).toBe(dummyTab.artist);
    expect(result.tab).toBe(dummyTab.tab);
    expect(result.url).toBe(dummyTab.url);
